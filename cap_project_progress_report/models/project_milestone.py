...
class ProjectMilestone(models.Model):
    ...
    status = fields.Selection([('not_started', 'Not Started'),
                               ...
                               ('feedback', 'Feedback'),
                               ('done', 'Done')], "Status")
    project_progress_id = fields.Many2one("project.progress", string="Project Progress Report")


class ProjectMilestoneHistory(models.Model):
    _name = 'project.milestone.history'
    _description = "Project Milestone"
    _inherit = ['mail.thread']
    _order = 'deadline, is_reached desc, name'

    def _get_default_project_id(self):
        return self.project_id.id or self.env.context.get('default_project_id') or self.env.context.get('active_id')

    @api.model_create_multi
    def create(self, vals):
        res = super(ProjectMilestoneHistory, self).create(vals)
        if not self.env.context.get('old_move'):
            self.env['project.milestone'].create({
                'name': res.name,
                'project_id': res.project_id.id,
                'is_reached': res.is_reached,
                'reached_date': res.reached_date,
                'status': res.status,
    ...
    def _get_fields_to_export(self):
        return ['id', 'name', 'start_date', 'deadline', 'is_reached', 'reached_date', 'is_deadline_exceeded',
                'is_deadline_future']
